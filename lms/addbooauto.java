...
public class addbooauto {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					addbooauto window = new addbooauto();
	...
	private void initialize() {
				...
				if (g.getSource() == btnAdd) {
					...
					try {
						...
						java.sql.PreparedStatement ps = con.prepareStatement(
								"insert into addbookauto(name,id,publisher,price,totalpages) values(?,?,?,?,?)");
						ps.setString(1, dn);
						ps.setString(2, dn1);
						ps.setString(3, dn2);
						ps.setString(4, dn3);
		...
		lblNewLabel.setIcon(new ImageIcon(addbooauto.class.getResource("/SS/IMG-20190817-WA0017.jpg")));
